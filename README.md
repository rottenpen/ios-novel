...
一个使用 SwiftUI 构建的 iOS 小说阅读器。支持导入 JSON 书源、多书源搜索、发现页荐书、书架管理和横向逐页阅读。

## 阅读体验

- 左右滑动翻页，点击左侧上一页、右侧下一页，中间打开阅读菜单。
- 章末进入下一章，章首可返回上一章末页；阅读进度保存到章内文字位置。
- 页脚显示章节进度、当前时间和章内页码；章节标题左对齐，与正文边缘一致。
- 冷启动自动打开上次在读的书并续看，依赖本地目录缓存、无需等待网络；可在设置页关闭。
- 支持主题、字号、行距、边距和常亮设置；调整排版或旋转屏幕后继续定位原文。
- 长按正文可复制本页内容；支持辅助功能朗读和翻页操作。
- 自动缓存已读正文，相邻章节预加载；已有缓存可在离线或书源缺失时阅读。
- 支持整本离线下载：详情页菜单可下载全本或从当前章往后下载，书架长按可直接下载全本。
  下载并发 4 章，已缓存章节自动跳过（再点一次即断点续传），失败章节可单独重试；书架顶部显示进度并可随时停止，已下载部分保留。
- 发现页内置精选书单（分潜力/养肥/完本三档，附标签、评分与简评），点书直达多书源搜索。
- 书架菜单进入设置页：集中管理阅读默认排版（主题/字号/行距/边距/常亮）、启动自动续看开关、缓存占用查看与一键清除，以及书源数量和版本信息。
